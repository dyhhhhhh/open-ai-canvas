...
func Builtins() *Registry {
	...
	registry, err := NewRegistry(
		...
		openAIVideosAdapter(), newAPIChannel1Adapter(), newAPIVideosAdapter(), xAIVideosAdapter(), arkVideosAdapter(), jimengVideosAdapter(), geminiVeoAdapter(), novitaVideosAdapter(), miniMaxVideosAdapter(), miniMaxH3VideosAdapter(),
		openAIAudioAdapter(), asyncAudioAdapter(), volcenginePlanTTSAdapter(), agnesAdapter(),
	)
	if err != nil {
		panic(err)
	}
...
func miniMaxH3VideosAdapter() Adapter {
	info := metadata("minimax-h3", "MiniMax H3", "MiniMax", CapabilityVideo, "POST /videos", "GET /videos/{task_id}", "application/json or multipart/form-data")
	info.Parameters = videoParams()
	// H3 accepts both public URLs and uploaded multipart references. The host
	// adapter owns that dual transport so media stays inside the resource boundary.
	return builtinAdapter{info: info}
}
...
func volcenginePlanTTSAdapter() Adapter {
	info := metadata("volcengine-plan-tts", "火山 Agent Plan 语音", "Volcengine Ark", CapabilityAudio, "POST /api/v3/plan/tts/unidirectional", "", "application/json + X-Api-*")
	info.Parameters = []Parameter{
		{Name: "model", Type: "string", Required: true, Mapping: "X-Api-Resource-Id", Description: "Agent Plan 资源标识，例如 seed-tts-2.0。"},
		{Name: "prompt", Type: "string", Required: true, Mapping: "req_params.text", Description: "待合成文本。"},
		{Name: "audioVoice", Type: "string", Mapping: "req_params.speaker", Description: "火山语音音色标识。"},
		{Name: "audioFormat", Type: "string", Mapping: "req_params.audio_params.format", Description: "输出格式，支持 mp3、wav、pcm、ogg_opus。"},
		{Name: "audioSpeed", Type: "number", Mapping: "req_params.audio_params.speech_rate", Description: "平台语速换算后限制在 -50 到 100。"},
	}
	// Agent Plan returns JSON, NDJSON or raw audio depending on the account.
	// Its decoder and X-Api headers are implemented by the host adapter.
	return builtinAdapter{info: info}
}

...
// processCanvasTextGenerationTask keeps task-owned text chunks durable so a page reload can recover the draft.
// The provider's stream parsing remains centralized in runTextTask to preserve its protocol-specific behavior.
...
func runImageTask(ctx context.Context, input canvasGenerationInput) (map[string]interface{}, error) {
	if input.Config.InterfaceType == string(model.ChannelInterfaceGrokImage) || input.Config.InterfaceType == string(model.ChannelInterfaceXAIImageLegacy) {
...
func validateGenerationInterface(mode string, interfaceType string) error {
	...
	allowed := map[string]map[string]bool{
		...
		"image": {"openai-image": true, "grok-image": true, "xai-image": true, "volcengine-ark-image": true, "volcengine-jimeng-image": true},
